...
Moreover, there is almost no standardization of these annotations themselves, even across studies performed by the same researcher.
...
In addition to enriching datasets and enabling dialog between researchers, we expect these annotations to provide an index target for additional discovery capabilities for users looking for particular features within data.
...
The Databrary website was opened for general use in October 2014 and accepting contributions since earlier that year.
...
\subsection{Normalization vs. Flexibility}
...
%%dg this might or might not be valuable but I am wondering if this is a section that might help draw all the pieces together. Or maybe this just boils down to a sentence or two in the next steps or conclusions
...
coding:
We are also working to support import and export of these annotations so researchers can continue to use their existing tools.
By supporting temporally-positioned session annotations directly within our system, researchers will be able to work with their recordings directly in raw form without having to transfer or transform it.
