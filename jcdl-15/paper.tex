...
Moreover, recordings are a primary source of raw data for scientific research in psychology, linguistics, education, anthropology, and many other disciplines.
Recordings collected in one context can often be used by others for different purposes with minimal explanation. 
This, therefore, naturally reduces the problems of documentation and provenance that plague many flat-file datasets and data repositories, especially those containing data that requires proprietary software to access.
Yet researchers rarely share recordings with others, due largely to entrenched practices and perceived requirements associated with privacy restrictions. %something here or somewhere else about aversion (fear) to changing practices...
Despite the inherent reusability of recordings, this lack of sharing inhibits the reuse of raw data and impedes scientific discovery.
...
Recordings can be and largely are consumed and analyzed, at least initially, without the use of sophisticated tools. Once recordings are digitally decoded, trained human observers can directly glean much of the data's richness.
Since automated analysis of videos is an active yet still immature research area, the problems of search, discoverability, and meta-analysis over these media remain \cite{Albertson_2013, Lanagan_Smeaton_2012}.
%^either need to say more here about annotations or do it in approach. 
...
\subsection{Approach}
%How databrary uniquely can deal with this problem because of its focus on video: by focusing on specific domain and focusing primarily on video, and inferring structure from actual data, databary has figured out how to standardize enough of the contextual metadata to facilitate understanding/enable re-use. 
...
%our novel contribution to this is the identification of generalizable set of principles around which to structure this contextual metadata, which enables re-use of recordings at an unprecendented scale. 
%annotating data is unique advantage of video, hope to leverage add'l data generated during analysis (i.e. annotations) to solve remaining problem is search.
...
The combination of these two elements allows data and metadata to be flexibly related and annotated. %another word besides annotated here?

\subsection{Sessions}

The session draws upon documented practices in the ways researchers use video.
It was designed to organize recordings and related data streams in a way familiar to researchers, keeping data in its most raw and flexible form without requiring  modifications that reduce the precision or utility of the data to others.
...
%for brevity, maybe this paragraph can go. it's adding nuance but does it advance our point about contextual metadata, structure and reuse? yes, because reuse is harder with only composite. 
...
% for brevity, can we either exclude or condense this sentence?: A dataset may consist of distinct sub-sets of participants based on differences in experimental procedures or locations; here each session would be placed into a record representing the procedure or location.
...
%%dg - reflagging this last sentence, it's valuable, but seems to do a major belly flop right here.
...
%%dg - Reading this again, this section seems a little redudant with what is discussed above with making records and sessions flexible? Might be able to sum up this section and the discovery section in a single 'Lessons learned and Upcoming Challenges' section
